...
open class Theme: NSObject {
	...
	*/
	open func setCodeFont(_ font: RPFont, reloadOther reload: Bool = false)
	{
		codeFont = font
		if reload
        {
	...
	internal func applyStyleToString(_ string: String, styleList: [String]) -> NSMutableAttributedString
	{
		...
		else if let font = codeFont
		{
			returnString = NSMutableAttributedString(string: string, attributes: [.font: font])
		}
		else
		{
